...
void effplot_RM(Int_t pmtnum){
  ...
  TString inthres;
  inthres = Form("M1R_threshold_pmt%d.dat",pmtnum);  // threshold data

  TString intwidth;
  intwidth = Form("M1R_tdcwidth_pmt%d.dat",pmtnum);  // tdc width cut data
  ...
  */
  ...
  TFile *outplot;
  TString outname = Form("M1R_plots_efficiency_pmt%d.root",pmt);
