...
void effplot_RM(Int_t pmtnum){
  ...
  TString inthres;
  inthres = Form("M3R_threshold_pmt%d.dat",pmtnum);  // threshold data

  TString intwidth;
  intwidth = Form("M3R_tdcwidth_pmt%d.dat",pmtnum);  // tdc width cut data
  ...
  */
  ...
  TFile *outplot;
  TString outname = Form("M5-R/M5R_plots_efficiency_pmt%d.root",pmt);
