...
//   Float_t aaft[14] = {294.0,295.0,306.5,292.0,269.0,281.0,297.0,276.0,287.0,327.0,319.0,284.0,300.0,342.0};  // ADC after
  ...
  TH1F *mdf = new TH1F("mdf","Comparison of Mean ADCs - PMT5",15,0,15);
  mdf->SetMaximum(1200.0);
  mdf->SetMinimum(0.0);
  mdf->SetStats(0);
  ...
  lgto->AddEntry(g1, "run 1986: without resistors", "P");
  lgto->AddEntry(g2, "run 2025: optimized resistors", "P");
  //Use this line if you want to exclude outliers. Comment out if not needed
  //lgto->AddEntry("","paddle 3 at 1731 (out of range)","P");
  //lgto->AddEntry("","paddle 13 at 2248 (out of range)","P");
  ...
  // Save plot as PDF
  //s1->Print("M4-L/PMT5_adccomparison_plot.pdf");
