...
TCanvas *plot_tres(Int_t pmt=1, Int_t adc_cut=40, Int_t tdc_min=750, Int_t tdc_width=300){

        TString cut, draw, draw1, title;
        title.Form("run_%d_TRes",run);
        TCanvas *cTRes = new TCanvas("cTRes",title,xcanvas,ycanvas);
	title.Form("run_%d_TRes_Fit",run);
	TCanvas *cTResFit= new TCanvas("cTResFit",title,xcanvas,ycanvas);
	...
	Double_t aconstants[16], ameans[16], asigmas[16], afwhm[16], atres[16];
	Double_t bconstants[16], bmeans[16], bsigmas[16], bfwhm[16], btres[16];
        ...
        for(Int_t icounter=1;icounter<=NUMPIXEL;icounter++){
                ...
                htmpa[icounter - 1]->SetLineColor(kBlue);
                htmpb[icounter - 1]->SetLineColor(kRed);
                title.Form("Run %d TRes pmt %d, paddle %d: %d < tdc < %d",run,pmt,icounter,tdc_min,tdc_min+tdc_width);
                htmpa[icounter - 1]->SetTitle(title);
                htmpb[icounter - 1]->SetTitle(title);
        }
	...
	cTResFit->Clear();
        ...
        for (Int_t i=0; i<NUMPIXEL; i++){
          ...
          if(i != pixel1[pmt-1]-1 && i != pixel2[pmt-1]-1) {
            ...
            Int_t aentries = htmpa[i]->GetEntries();
            float amean = htmpa[i]->GetMean(1);
            float aRMS = htmpa[i]->GetRMS(1);

	    Int_t bentries = htmpb[i]->GetEntries();
            float bmean = htmpb[i]->GetMean(1);
            float bRMS = htmpb[i]->GetRMS(1);
            //cout << entries <<" "<< mean <<" "<< RMS <<endl;
	    ...
	    htmpa[i]->Fit("gaus","","",amean-200,amean+200);
	    htmpb[i]->Fit("gaus","","",bmean-200,bmean+200);

	    afunction = htmpa[i]->GetFunction("gaus");
	    afunction->SetLineColor(1);

	    aconstants[i] = afunction->GetParameter(0);
	    ameans[i] = afunction->GetParameter(1);
	    asigmas[i] = afunction->GetParameter(2);
	    afwhm[i] = 2.355*asigmas[i];
	    atres[i] = afwhm[i]*0.5;
	    	    
	    bfunction = htmpb[i]->GetFunction("gaus");
	    bfunction->SetLineColor(1);

	    bconstants[i] = bfunction->GetParameter(0);
	    bmeans[i] = bfunction->GetParameter(1);
	    bsigmas[i] = bfunction->GetParameter(2);
	    bfwhm[i] = 2.355*bsigmas[i];
	    btres[i] = bfwhm[i]*0.5;
	...
	for (Int_t i=0; i<NUMPIXEL; i++)
	  {
	    cout << "Pixel number: " << i+1 << "   \t atres = " << atres[i] << " (ns)    \t btres = " << btres[i] << " (ns)" << endl;
	  }
	
        title.Form("run_%d_TRes_pmt_%d_adc_cut_%d.png",
                   run,pmt,adc_cut);
        cTRes->Print(title);
        cTRes->cd(0);
        return cTRes;
