...
// Initialize Dark Body Radiator Palette
...
/*case 1000:
	   ...
	   Double_t stops[9] = {0.0, 0.1, 0.25, 0.4, 0.55, 0.7, 0.85, 0.9, 1.0};
	   TColor Idx = TColor::CreateGradientColorTable(number, stops, red, green, blue, 255, 1.0);
	   }*/
...
void plotL_JB(Int_t runno, Int_t n_events=-1){
...
TCanvas *plot_xheat(Int_t pmt=1, Int_t mod=1, Int_t rn=4, char side='L'){

        TString cut, draw, draw1, title;
	string tx,px;
	Int_t hv[NUMRUNS];
	Int_t runs[NUMRUNS];
	Int_t* pixels = new Int_t[NUMPIXEL];
        title.Form("run_%d_XTalk_PMT_%d",run,pmt);
       
	TCanvas *cXHeat = new TCanvas();      
	TCanvas *cHits = new TCanvas();
	TCanvas *cDHits = new TCanvas();

	Double_t xhits[NUMPIXEL] = new Double_t[NUMPIXEL];
	Double_t hits[NUMPIXEL] = new Double_t[NUMPIXEL];
	Double_t dhits[NUMPIXEL] = new Double_t[NUMPIXEL];

	Double_t thr[NUMRUNS][NUMPIXEL] = new Double_t[NUMRUNS][NUMPIXEL];
	Double_t threshs[NUMPIXEL] = new Double_t[NUMPIXEL];

	TString inthres;
	inthres = Form("M%d%c_threshold_pmt%d.dat",mod,side,pmt);  // threshold data
	cout << inthres << endl;
	TString intwidth;
	intwidth = Form("M%d%c_tdcwidth_pmt%d.dat",mod,side,pmt);  // tdc width cut data
	cout << intwidth << endl;
	...
	tmpentry.Form("hxheat");
        ...
        TH2D *hxheat = new TH2D(tmpentry,tmpentry,NUMPIXEL/4,0,NUMPIXEL/4,NUMPIXEL/4,0,NUMPIXEL/4);
        title.Form("XTalk: Module %d%c, PMT %d",mod,side,pmt);
        hxheat->SetTitle(title);
	hxheat->GetXaxis()->SetNdivisions(4);
	hxheat->GetYaxis()->SetNdivisions(4);

	TH2D *hxhits = new TH2D(tmpentry, tmpentry, NUMPIXEL/4, 0, NUMPIXEL/4, NUMPIXEL/4, 0, NUMPIXEL/4);
	title.Form("Total Hits: Module %d%c, PMT %d", mod,side,pmt);
	hxhits->SetTitle(title);
	hxhits->GetXaxis()->SetNdivisions(4);
	hxhits->GetYaxis()->SetNdivisions(4);
	
	TH2D *hdb_hits = new TH2D(tmpentry, tmpentry, NUMPIXEL/4, 0, NUMPIXEL/4, NUMPIXEL/4, 0, NUMPIXEL/4);
	title.Form("Double Hits: Module %d%c, PMT %d", mod,side,pmt);
	hdb_hits->SetTitle(title);
	hdb_hits->GetXaxis()->SetNdivisions(4);
	hdb_hits->GetYaxis()->SetNdivisions(4);

	hxhits->SetMinimum(0);
	hxhits->SetMaximum(15000);
	hdb_hits->SetMinimum(0);
	hdb_hits->SetMaximum(15000);

	ifstream tt; 
	tt.open(inthres,ios::in);  
	if(tt.is_open()){
	  tt >> tx >> runs[0] >> runs[1] >> runs[2] >> runs[3] >> runs[4];
	  tt >> px >> pmt >> hv[0] >> hv[1] >> hv[2] >> hv[3] >> hv[4];
	  cout << px << "\t" << pmt << endl;
	  cout << tx << "\t" << runs[0] << "\t" << runs[1] << "\t" << runs[2] << "\t" << runs[3] << "\t" << runs[4] << endl;
	...
	Int_t tdcw[NUMRUNS][NUMPIXEL] = new Int_t[NUMRUNS][NUMPIXEL];   // 2D array TDC width cuts 
	Int_t twpx[NUMPIXEL] = new Int_t[NUMPIXEL]; 
	Int_t twidths[NUMPIXEL] = new Int_t[NUMPIXEL];
	...
	if(tw.is_open()){ getline(tw,tln); // get the first line of the file
	  ...
	  for(Int_t j=0;j<NUMPIXEL;j++){ 
	    if(j==16){break;} 
	    tw >> twpx[j] >> tdcw[0][j] >> tdcw[1][j] >> tdcw[2][j] >> tdcw[3][j] >> tdcw[4][j];
	    cout << twpx[j] << "\t" << tdcw[0][j] << "\t" << tdcw[1][j] << "\t" << tdcw[2][j] << "\t" << tdcw[3][j] << "\t" << tdcw[4][j] << endl;
	    twidths[j] = tdcw[rn][j];
	    if(j==15){break;} 
	  }
	} 
	tw.close();
	   ...
	   for(Int_t index = 1; index<=NUMPIXEL; index++){
	     ...
	     if(tdc>500){hits[pxl]+=1;}

	     for(Int_t j=1; j<=NUMPIXEL;j++){
	       Int_t ipad = (pmt-1)*NUMPIXEL+j-1;
	     ...
	     if(tdc>500 && adc>threshs[pxl] && tdiff<=twidths[pxl])
	       {
		 xhits[pxl]+=1;
	       }
	   }
	...
	//plot 3D histo
        ...
        for (Int_t i=0; i<4; i++){
          for (Int_t j=0; j<4; j++){
	    if(pix>15){break;}
	    hxhits->Fill(j,i,hits[pix]);
	    hxheat->Fill(j,i,xhits[pix]);
	    hdb_hits->Fill(j,i,dhits[pix]);
	    //cout << pix << "\t" << hits[pix] << endl;
	    //cout << pix << "\t" << xhits[pix] << endl;
	    //cout << pix << "\t" << dhits[pix] << endl;
	    pix++;
	  }
        }

	cXHeat->SetName("cXHeat");
	cXHeat->cd();
	hxheat->Draw("COLZ");
	hxheat->Draw("TEXT SAME");
	hxheat->GetXaxis()->SetTitle("Horizontal Pixel Number");
	hxheat->GetYaxis()->SetTitle("Vertical Pixel Number");
	title.Form("run_%d_module_%d_pmt_%d_xhits.pdf",run,mod,pmt);
	cXHeat->Print(title);

	cHits->SetName("cHits");
	cHits->cd();
	hxhits->Draw("COLZ");
	hxhits->Draw("TEXT SAME");
	hxhits->GetXaxis()->SetTitle("Horizontal Pixel Number");
	hxhits->GetYaxis()->SetTitle("Vertical Pixel Number");
	title.Form("run_%d_module_%d_pmt_%d_hits.pdf",run,mod,pmt);
	cHits->Print(title);

	cDHits->SetName("cDHits");
	cDHits->cd();
	hdb_hits->Draw("COLZ");
	hdb_hits->Draw("TEXT SAME");
	hdb_hits->GetXaxis()->SetTitle("Horizontal Pixel Number");
	hdb_hits->GetYaxis()->SetTitle("Vertical Pixel Number");
	title.Form("run_%d_module_%d_pmt_%d_dhits.pdf",run,mod,pmt);
	cDHits->Print(title);
...
TCanvas *plot_xpercent(Int_t pmt=1, Int_t mod=1, Int_t rn=4, char side='L'){
	...
	Int_t* pixels = new Int_t[NUMPIXEL];
        title.Form("run_%d_XTalk_PMT_%d",run,pmt);
        TCanvas *cXPer = new TCanvas();          //Producing weird initial canvas
	TCanvas *cDPer = new TCanvas();
	TCanvas *cXDPer = new TCanvas();
	...
	tmpentry.Form("hxheat");
        ...
        TH2D *hxper = new TH2D("hxper","hxper",NUMPIXEL/4,0,NUMPIXEL/4,NUMPIXEL/4,0,NUMPIXEL/4);
        title.Form("XTalk (%): Module %d%c, PMT %d",mod,side,pmt);
        hxper->SetTitle(title);
	hxper->GetXaxis()->SetNdivisions(4);
	hxper->GetYaxis()->SetNdivisions(4);

	TH2D *hdper = new TH2D("hdper","hdper", NUMPIXEL/4, 0, NUMPIXEL/4, NUMPIXEL/4, 0, NUMPIXEL/4);
	title.Form("Double Hits (%): Module %d%c, PMT %d", mod,side,pmt);
	hdper->SetTitle(title);
	hdper->GetXaxis()->SetNdivisions(4);
	hdper->GetYaxis()->SetNdivisions(4);

	TH2D *hxdper = new TH2D("hxdper","hxdper", NUMPIXEL/4, 0, NUMPIXEL/4, NUMPIXEL/4, 0, NUMPIXEL/4);
	title.Form("XTalk Double Hits (%): Module %d%c, PMT %d", mod,side,pmt);
	hxdper->SetTitle(title);
	hxdper->GetXaxis()->SetNdivisions(4);
	hxdper->GetYaxis()->SetNdivisions(4);
	...
	//plot 3D histo
        ...
        for (Int_t i=0; i<4; i++){
          for (Int_t j=0; j<4; j++){
	    if(pix>15){break;}
	    hxper->Fill(j,i,(xhits[pix]/hits[pix])*100);
	    hdper->Fill(j,i,(dhits[pix]/hits[pix])*100);
	    hxdper->Fill(j,i,(xhits[pix]/dhits[pix])*100);
	    //cout << pix << "\t" << xhits[pix] << endl;
	    pix++;
	  }
        }
	...
	cXPer->SetName("cXPercent");
	cXPer->cd();
	hxper->Draw("COLZ");
	hxper->Draw("TEXT SAME");
	hxper->GetXaxis()->SetTitle("Horizontal Pixel Number");
	hxper->GetYaxis()->SetTitle("Vertical Pixel Number");
	title.Form("run_%d_module_%d_pmt_%d_xper.pdf",run,mod,pmt);
	cXPer->Print(title);
	
	cDPer->SetName("cDPer");
	cDPer->cd();
	hdper->Draw("COLZ");
	hdper->Draw("TEXT SAME");
	hdper->GetXaxis()->SetTitle("Horizontal Pixel Number");
	hdper->GetYaxis()->SetTitle("Vertical Pixel Number");
	title.Form("run_%d_module_%d_pmt_%d_dper.pdf",run,mod,pmt);
	cDPer->Print(title);
	
	cXDPer->SetName("cXDPer");
	cXDPer->cd();
	hxdper->Draw("COLZ");
	hxdper->Draw("TEXT SAME");
	hxdper->GetXaxis()->SetTitle("Horizontal Pixel Number");
	hxdper->GetYaxis()->SetTitle("Vertical Pixel Number");
	title.Form("run_%d_module_%d_pmt_%d_xdper.pdf",run,mod,pmt);
	cXDPer->Print(title);
...
TCanvas *plot_adc_fit(Int_t pmt=1, Int_t tdc_min=750, Int_t tdc_width=400, Int_t adc_neighbor_cut=30, Int_t adc_cut=40){
...
TCanvas *plot_tdc_fit(Int_t pmt=1, Int_t pixel=1, Int_t highv=800, Int_t tdc_cut=500, Int_t adc_neighbor_cut=20, 
	...
	if(count2>5){
	  g2->Draw("same P");
	  g2->Fit("f1","B"); // red long-dashed line
	  Double_t diff = g2->GetFunction("f1")->GetParameter(0);
	  Double_t slope = g2->GetFunction("f1")->GetParameter(1);

	cout << "fit parameters for g2: slope = " << slope << "\t intercept = " << diff << endl;

	  Double_t ymin = 750+diff;
	  Double_t xmax = 750+((1020-ymin)/(slope));
	  Double_t tdc_width_cut = diff+shift;

	cout << "ymin = " << ymin << "\t xmax = " << xmax << "\t tdc width cut" << tdc_width_cut << endl;

	  TLine *lg = new TLine(750.0,ymin+shift,xmax,1020+shift); // green dotted line
	  lg->SetLineColor(8);
	  lg->SetLineStyle(2);
	  lg->SetLineWidth(3);
	  lg->Draw();
	...
	cout << "Graph counts: g1 = " << count1 << "\t g2 = " << count2 << "\t g3 = " << count3 << endl;
