...
TCanvas *plot_xheat(Int_t pmt=1, Int_t mod=1, Int_t rn=4, TString side="L"){
	...
	TString inthres;
	inthres = Form("M%dL_threshold_pmt%d.dat",mod,pmt);  // threshold data
	cout << inthres << endl;
	TString intwidth;
	intwidth = Form("M%dL_tdcwidth_pmt%d.dat",mod,pmt);  // tdc width cut data
	cout << intwidth << endl;
	...
	title.Form("run_%d_module_%d_pmt_%d.pdf",run,mod,pmt);
	cXHeat->Print(title);
	...
	hxhits->GetXaxis()->SetTitle("Column");
	hxhits->GetYaxis()->SetTitle("Row");
	title.Form("Before Cuts");
	cHits->Print(title);
	...
	hdb_hits->GetXaxis()->SetTitle("Column");
	hdb_hits->GetYaxis()->SetTitle("Row");
	title.Form("Double Hits");
	cDHits->Print(title);
...
TCanvas *plot_xpercent(Int_t pmt=1, Int_t mod=1, Int_t rn=4, TString side="L"){
	...
	Int_t* pixels = new Int_t[NUMPIXEL];
        title.Form("run_%d_XTalk_PMT_%d",run,pmt);
        TCanvas *cXHeat = new TCanvas();          //Producing weird initial canvas
	TCanvas *cHits = new TCanvas();
	...
	TH2D *hxhits = new TH2D(tmpentry, tmpentry, NUMPIXEL/4, 0, NUMPIXEL/4, NUMPIXEL/4, 0, NUMPIXEL/4);
	title.Form("TotalHits: Module %dL, PMT %d", mod,pmt);
	hxhits->SetTitle(title);
	hxhits->SetMinimum(0);
	hxhits->SetMaximum(15000);	
	...
	tw.close();
	   ...
	   for(Int_t index = 1; index<=NUMPIXEL; index++){
	     ...
	     if(tdc>500 && adc>threshs[pxl] && tdiff<=twidths[pxl])
	       {
		 //cout << tdc << "\t" << adc "\t" << threshs[pxl] << "\t" << tdiff << "\t" << twidths[pxl] << endl;
		 xhits[pxl]+=1;
	       }
	   }
	}
	...
	//plot 3D histo
	    ...
	    if(pix>15){break;}
	    hxhits->Fill(j,i,hits[pix]);
	    hxheat->Fill(j,i,(xhits[pix] / hits[pix]));
	    cout << pix << "\t" << xhits[pix] << endl;
	    pix++;
	  }
        }
