     ...
     rolParam rolp;
{
      ...
      case 0 :
	{
	  ...
	  strcat(name, ":pool");
	  rolp->pool  = partCreate(name, 8192  , 1024 ,1);
	  if (rolp->pool == 0) {
	    rolp->inited = -1;
	    break;
	  }
...
static void __download()
{
  ...
  if (sysLocalToBusAdrs(0x09,0,&sfi_cpu_mem_offset)) { 
    ...
    *(rol->async_roc) = 0;  
  {   
unsigned long adc_id, res, laddr, jj, kk;
bigendian_out = 0;
{ 
...
{ 
{
  ...
  tiSetTriggerSourceMask((1<<5)  | (1<<4)  | (1<<2) );
  tiEnableTSInput( (1<<0) );
...
if(( pedsuppress == 1) ) {
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x40000082,1,1,1,0,0,1,1);
}
else{
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x00000082,1,1,1,0,0,1,1);
} 
    sadr = 7 ;
...
if(( pedsuppress == 1) ) {
  ...
  sfi_error_decode(0);
    padr   = 16 ; 
    fb_fwc_1(padr,0,0x00000900,1,1,0,1,0,1,1);
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x60000003,1,1,1,0,0,1,1);
    sadr = 18 ;
    fb_fwc_1(0,sadr,0x00008006,1,1,1,0,0,1,1);
    sadr = 7 ;
    fb_fwc_1(0,sadr,2,1,1,1,0,0,1,1);
    fprel(); 
  sfi_error_decode(0);
    padr   = 15 ; 
    fb_fwc_1(padr,0,0x00001900,1,1,0,1,0,1,1);
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x60000003,1,1,1,0,0,1,1);
    sadr = 18 ;
    fb_fwc_1(0,sadr,0x00008006,1,1,1,0,0,1,1);
  ...
  sfi_error_decode(0);
    padr   = 14 ; 
    fb_fwc_1(padr,0,0x00001100,1,1,0,1,0,1,1);
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x60000003,1,1,1,0,0,1,1);
    sadr = 18 ;
    fb_fwc_1(0,sadr,0x00008006,1,1,1,0,0,1,1);
...
StartOfEvent[event_depth__++] = (rol->dabufp); if(input_event__) {	*(++(rol->dabufp)) = (( 1 ) << 16) | (( 0x01 ) << 8) | (0xff & 
(input_event__->nevent));	} else {	*(++(rol->dabufp)) = (syncFlag<<24) | (( 1 ) << 16) | (( 0x01 ) << 8) | (0xff & 
...
{ 
  ...
  fb_frcm_1(9,0,&datascan,1,0,1,0,0,0);
  for (jj=0;jj<= 7 ;jj++) {
     if(datascan & (1<<modslots[jj])) {
       padr=modslots[jj];
       fb_fwc_1(padr,0,0x400,1,1,0,1,0,0,0);
       logMsg("titrg %d %d %d %x\n",evcount,jj,modslots[jj],datascan);
     }
  }
 } 
}
