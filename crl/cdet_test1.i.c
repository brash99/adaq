...

{
  ...
  for(imodule=0;imodule<nmodules;imodule++){
    ...
    for(ichan=0;ichan< 64 ;ichan++){
      ...
      temp = 1000+thresholds[imodule][ichan];
...
{ 
  ...
  sfi_error_decode(0);
    ...
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x00000041,1,1,1,0,0,1,1);
...
void titrig(unsigned long EVTYPE,unsigned long EVSOURCE)
{
    long EVENT_LENGTH;
  {   
unsigned long dCnt, ev_type, ii, datascan, res, jj, fbres;
