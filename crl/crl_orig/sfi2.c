...
unsigned long ii, jj, kk, datascan, event_ty, event_no, slot_num, fbres;
...
*(rol->nevents));	}	((rol->dabufp))++;} ; {	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 1 ) << 16) | ( 0x01 ) 
<< 8) | ( 0 );	((rol->dabufp))++; ;     {*(rol->dabufp)++ = ( 0x0da00ddc4 );} ; 
*rol->dabufp++ = *t1;
*rol->dabufp++ = *t2;
ii = 0 ;
datascan = 0 ;
...
{ 
   for (jj=0;jj<= 3 ;jj++) {
    ...
    {*(rol->dabufp)++ = ( 0xfabc000a );} ; 
   icnt = icnt + 1;
   if(icnt > 20000) icnt = 0;
   *rol->dabufp++ = icnt;
    {*(rol->dabufp)++ = ( ii );} ; 
    {*(rol->dabufp)++ = ( datascan );} ; 
    {*(rol->dabufp)++ = ( 0xfabc000b );} ; 
{ 
   if (buffered) { 
...
<< 8) | ( 0 );	((rol->dabufp))++; ;   *rol->dabufp++ = 0x12345888;
...
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ;     }
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 3 ) << 16) | ( 0x01 ) 
<< 8) | ( 0 );	((rol->dabufp))++; ;   *rol->dabufp++ = 0x12345678;
*StartOfBank = (long) (((char *) (rol->dabufp)) - ((char *) StartOfBank));	if ((*StartOfBank 
& 1) != 0) { (rol->dabufp) = ((long *)((char *) (rol->dabufp))+1); *StartOfBank += 1; }; if 
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ; }
