...
struct TI_A24RegStruct
{
    ...
    volatile unsigned int fpDelay[2];
             ...
             unsigned int blank5[(0x180-0x12C)/4];
...
input_event__ = (DANODE *) 0; } ;     *(rol->nevents) = 0;
  {   
unsigned long jj, adc_id, kk, slot_num, csr_0, pedsuppress, tslot;
adc_id = 0 ;
  pedsuppress=0;
...
void titrig(unsigned long EVTYPE,unsigned long EVSOURCE)
{
    long EVENT_LENGTH;
  {   
unsigned long ii, jj, datascan, event_ty;
...
*(rol->nevents));	}	((rol->dabufp))++;} ;     {*(rol->dabufp)++ = ( 0x0da00ddc4 );} ; 
ii = 0 ;
datascan = 0 ;
