     ...
     rolParam rolp;
{
      ...
      case 0 :
	{
	  ...
	  strcat(name, ":pool");
	  rolp->pool  = partCreate(name, 8192  , 256 ,1);
	  if (rolp->pool == 0) {
	    rolp->inited = -1;
	    break;
	  }
...
static void __download()
{
  ...
  if (sysLocalToBusAdrs(0x09,0,&sfi_cpu_mem_offset)) { 
    ...
    *(rol->async_roc) = 0;  
  {   
unsigned long res, laddr, jj, kk, mm;
bigendian_out = 0;
{ 
...
input_event__ = (DANODE *) 0; } ;     *(rol->nevents) = 0;
  {   
unsigned long pedsuppress, kk, tslot;
...
if(( pedsuppress == 1) ) {
  ...
  sfi_error_decode(0);
    ...
    fb_fwc_1(0,0,0x00000900,1,1,1,1,0,1,1);
    sadr = 1 ;
...
while( ( kk <  3 -1) ){ 
    ...
    padr   = tslot; 
    fb_fwc_1(padr,0,0x40000000,1,1,0,1,0,1,1);
    fb_fwc_1(0,0,0x00001900,1,1,1,1,0,1,1);
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x40000003,1,1,1,0,0,1,1);
    sadr = 18 ;
    fb_fwc_1(0,sadr,0xbb6,1,1,1,0,0,1,1);
    ...
    padr   = 18 ; 
    fb_fwc_1(padr,0,0x40000000,1,1,0,1,0,1,1);
    fb_fwc_1(0,0,0x00001100,1,1,1,1,0,1,1);
    sadr = 1 ;
    fb_fwc_1(0,sadr,0x40000003,1,1,1,0,0,1,1);
    sadr = 18 ;
    fb_fwc_1(0,sadr,0xbb6,1,1,1,0,0,1,1);
...
if (fbres) logMsg("(A) fbres = 0x%x datascan = 0x%x scan_mask = 
0x%x\n",fbres,datascan,scan_mask,0,0,0);   if ((datascan != 0) && (datascan&~scan_mask)) { 
...
{ 
  datascan = 0;
  fbres = fb_frcm_1(9,0,&datascan,1,0,1,0,0,0);
if (fbres) logMsg("(B) fbres = 0x%x datascan = 0x%x scan_mask = 
0x%x\n",fbres,datascan,scan_mask,0,0,0);   if ((datascan != 0) && (datascan&~scan_mask)) { 
