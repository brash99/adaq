...
void sfi1_ts_blocking__init (rolp)
     rolParam rolp;
{
      ...
      case 0 :
	{
	  ...
	  strcat(name, ":pool");
	  rolp->pool  = partCreate(name, 16800  , 100 ,1);
	  if (rolp->pool == 0) {
	    rolp->inited = -1;
	    break;
	  }
...
void titrig(unsigned long EVTYPE,unsigned long EVSOURCE)
{
    long EVENT_LENGTH;
  {   
unsigned long dCnt, ev_type, ii, datascan, res, jj, fbres, numLocal, numBranchdata, syncFlag, 
mblk;   *sfi.sequencerEnable = 0;
  rol->dabufp = (long *) 0;
{ 
{
  if(1 ==1)
