...
public class Terminal extends Canvas implements ClassConnection {
  ...
  /* Robot to simulate keyboard keypressing */ /*** 13 Feb 2015 ***/
    Robot robot; 
...
*/
  ...
  public synchronized void screenClean(Graphics g) {
    if( DEBUG ) System.out.println("Terminal: Cleaning screen: " + dirtyTop +
				   " to " + dirtyBottom);
  ...
  public synchronized void write(char[] d, int off, int len, int flags) {
    ...
    if( DEBUG ) {
      ...
      System.out.println("Terminal: Write: " + new String(d,off,len));
    }
    if( len == 1 ) {
      write(d[off],flags);
    } else if( len > 1 ) {
  ...
  boolean put_char(char c) {
    ...
    if( curCol >= regionRight && !overChar ) {
      ...
      curCol = regionRight;
      overChar = true;
      if( DEBUG ) System.out.println("Terminal: Hit margin, overChar=" + overChar);
	...
	*/
	...
	if( DEBUG ) System.out.println("Terminal: Past margin, overChar=" + overChar);
      } else {
	page[curRow][curCol] = c;
	style[curRow][curCol] = curStyle;
  ...
  public boolean handleEvent(Event e) {
    ...
    //    System.out.println("Terminal: return from handelEvent()");
