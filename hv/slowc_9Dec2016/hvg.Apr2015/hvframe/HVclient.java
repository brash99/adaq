/* 
 * Update 20-Apr-2015: add variable 'host=f.host' and replaced in messagews 'server' on 'host'
...
public class HVclient extends Thread  {
  ...
  // constructor 
	...
	try {
	    ...
	    status ="Couldn't get I/O for the connection to:" + host;
            System.err.println(status);
	    errStatus = true;
	    notifys.updateStatus(status);
	    // System.exit(1);
	...
	try {
	    close() ;
            ...
            System.err.println("Couldn't close the connection to:" + host);
            System.exit(1);
        }
	while ((!fgetconnection)&&(frozen)) {
		...
		cnt ++;
		if(cnt>9) {
		    System.err.println("Couldn't get I/O for the connection to:"+host +" after "+cnt+" attemps");
		    notifys.updateStatus("Couldn't get I/O for the connection to:"+host +" after "+cnt+" attemp");	
		    cnt =0;
		    //frozen = false;
		    //System.exit(1);
		}
	...
	do {
		...
		notifys.updateStatus("HVclient: Reopen connection to: "+host);	   
		System.out.println("HVclient: Reopen connection to: "+host);
	    }
		...
		System.out.println("Client to "+ host + ":" + fromUser);
		...
		do {
			...
			while(!hvin.ready()) {			    
				...
				errStatus = true;
				//frozen = false;
				notifys.updateStatus("ALARM : HVclient: NO response from server: "+host);
				hvmess.setConnected(false);
				boolean b = f.checkClientStatus();
