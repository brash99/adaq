...
// Trees
Int_t tree_adc_slot, tree_adc_chan, tree_adc_hits, tree_adc_pedestal_chan;
Int_t tree_tdc_slot, tree_tdc_chan, tree_tdc_hits;
Int_t isleadingedge;
...
int main(int argc, char* argv[])
{
  ...
  // Trees
  tree_adc = new TTree("tree_adc","Raw ADC data tree");
  tree_adc->Branch("runno"		  ,&runno,         "runno/I");
  tree_adc->Branch("event"		  ,&ievent,        "event/I");
  tree_adc->Branch("slot"	          ,&tree_adc_slot, "slot/I");
  tree_adc->Branch("chan" 	          ,&tree_adc_chan, "chan/I");
  tree_adc->Branch("hits"	          ,&tree_adc_hits, "hits/I");
  tree_adc->Branch("pedestal_chan"        ,&tree_adc_pedestal_chan,     "pedestal_chan/I");

  tree_tdc = new TTree("tree_tdc", "Raw TDC data tree");
  tree_tdc->Branch("run"		  ,&runno,         "run/I");
  tree_tdc->Branch("event"		  ,&ievent,        "event/I");
  tree_tdc->Branch("slot"	          ,&tree_tdc_slot, "slot/I");
  tree_tdc->Branch("chan" 	          ,&tree_tdc_chan, "chan/I");
  tree_tdc->Branch("hits"		  ,&tree_tdc_hits, "hits/I");
...
void decode (int* data) {
  ...
  for (int j = index+1; j < indexlast; j++) {
	...
	tree_adc_chan = ichan;
	tree_adc_hits = rdata;
	tree_adc_pedestal_chan = pedestal_threshold[ichan][slot];
...
void readpedsup(){
  ...
  for (int i = 0; i < NUMADCSLOTS;i++) {
    ...
    for (int j = 0; j < NUMCHANA;j++) {
      infile >>  thres;
      pedestal_threshold[j][slot] = atoi(thres);
      // cout << "readpedsup(): chan = "<< j<<" thres = '" << thres << "' value = " << pedestal_threshold[j][slot] <<endl;
    }
  }
