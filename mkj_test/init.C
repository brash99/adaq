...
// channels. Fills global vector for noise freq. Optionally, a
// threshold on tdc for noise calculation purposes may be supplied
...
// For interactive use:
// save results (retrieved from global arrays, see "format" below for
...
void save_data(float abs_NINOthreshold_V, float abs_PMT_HV_V, 
	       int adc_slot=-1, int chan_min=0, int chan_max=63, 
	       int tdc_slot=-1, int tdc_chan=-1){
  // defalt slot min/max
  int slot_min = 0;
  int slot_max = NUMADCSLOTS;
  if (adc_slot != -1) { // requested a particular slot
    slot_min = adc_slot;
    slot_max = adc_slot+1;
  }

  tdc_cut = last_tdc_cut;
  int tdc_chan_req = tdc_chan;

  // just in case...
  if (abs_NINOthreshold_V < 0) abs_NINOthreshold_V = -abs_NINOthreshold_V;
  if (abs_PMT_HV_V < 0 ) abs_PMT_HV_V = - abs_PMT_HV_V;
  // no, can't simply use abs(), ROOT's version is just FUBAR:
  // abs(  1.5 ) -> 1
  // abs( -1.5 ) -> 1

  // time to save...
  char filename[100];
  sprintf(filename,"run_%d_NINOthreshold_minus_%f_V_PMT_minus_%f_V.txt",
	  run, abs_NINOthreshold_V, abs_PMT_HV_V);
  ...
  // header
  format = "#adc_slot, chan, Athreshold, Awidth, peak_mean, peak_rms, Nphe, gain, noise_freq";
  cout << format << endl;
  file << format <<endl;
  file <<"#NINO threshold "<< -abs_NINOthreshold_V 
       <<" V, PMT HV "<< -abs_PMT_HV_V<<endl;

  for (int slot = slot_min; slot < slot_max; slot++){
    for (int chan = chan_min; chan < chan_max+1; chan++){
      if (tdc_slot == -1) tdc_slot = calc_tdc_slot(slot,chan);
      if (tdc_chan_req == -1) {
	tdc_chan = calc_tdc_chan(slot,chan);
      } else { // requested a particular start chan for tdc
	tdc_chan = tdc_chan_req + chan - chan_min;
      }

      format.Form("%d %d %.3f %.3f %.3f %.3f %.3f %.3f %.3f"
		  ,slot, chan 
		  ,Athresholds[slot][chan], Awidths[slot][chan]
		  ,peak_mean(slot,chan), peak_rms(slot,chan)
		  ,phe(slot,chan), gain(slot,chan)
		  ,noise_freq(tdc_slot,tdc_chan, tdc_cut)
		  );
      // note that arguments to noise_freq assume tdc slot 0,chans 0-15,cut=1350
      file << format << endl;
      cout << format << endl;
    }
  }
...
TCanvas *plot_walk(int adc_slot, int adc_chan_start, int tdc_slot=-1, int tdc_chan_start=-1, int adc_min=0, int adc_max=600, int deltat_min=0, int deltat_max=120){
  TString draw, cut, title;

  TCanvas *cWALK = new TCanvas("cWALK","cWALK");
  cWALK->Divide(4,4);

  char tmpentry[25];

  if (tdc_slot == -1) tdc_slot = calc_tdc_slot(adc_slot,adc_chan);
  if (tdc_chan_start == -1) tdc_chan_start = calc_tdc_chan(adc_slot,adc_chan);

  for (int adc_chan = adc_chan_start; adc_chan < adc_chan_start+16; adc_chan++){
    cWALK->cd(adc_chan - adc_chan_start +1);
    tdc_chan = tdc_chan_start + adc_chan - adc_chan_start;

    draw.Form("adc[%d][%d]:tdcl[%d][%d]-tdct[%d][%d]>>htemp"
	      ,adc_slot,adc_chan, tdc_slot,tdc_chan, tdc_slot,tdc_chan);
    cut.Form("adc[%d][%d]>%d && adc[%d][%d]<%d && tdct[%d][%d]>0 && tdcl[%d][%d]>0 && tdcl[%d][%d]-tdct[%d][%d]>%d && tdcl[%d][%d]-tdct[%d][%d]<%d"
	     ,adc_slot,adc_chan,adc_min, adc_slot,adc_chan,adc_max
	     ,tdc_slot,tdc_chan,tdc_slot,tdc_chan
	     ,tdc_slot,tdc_chan, tdc_slot,tdc_chan, deltat_min
	     ,tdc_slot,tdc_chan, tdc_slot,tdc_chan, deltat_max);
    title.Form("Run %d adc slot %d channel %d",run,adc_slot,adc_chan);
    
    TH2D *htemp = new TH2D("htemp","htemp"
			   ,deltat_max-deltat_min, deltat_min, deltat_max
			   ,adc_max-adc_min, adc_min, adc_max);
    
    t->Draw(draw,cut,"prof");
    
    htemp->GetXaxis()->SetTitle("Delta t (TDC channels)");
    htemp->GetYaxis()->SetTitle("ADC amplitude");
    htemp->SetTitle(title);
    htemp->Draw();
  
    int entries = htemp->GetEntries();
    float meanx = htemp->GetMean(1);
    float meany = htemp->GetMean(2);
    float RMSx = htemp->GetRMS(1);
    float RMSy = htemp->GetRMS(2);

    int htmin = htemp->GetXaxis()->GetXmin();
    int htmax = htemp->GetXaxis()->GetXmax();
    int hamin = htemp->GetYaxis()->GetXmin();
    int hamax = htemp->GetYaxis()->GetXmax();
    
    int nbinst = htemp->GetXaxis()->GetNbins();
    int nbinsa = htemp->GetYaxis()->GetNbins();
    float ampt = (htmax-htmin)/nbinst;
    float ampa = (hamax-hamin)/nbinsa;
    
    float worst_relerror = 0, worst_dt=-1000;
    int local_min, local_max, content;
    float mean, vadc, local_relerror;
    int npoints;
    for (int bint=1; bint<nbinst; bint++){
      local_min=hamax+100; local_max=hamin-100;
      mean = 0.;
      npoints = 0;
      for (int bina=1; bina<nbinsa; bina++){
	content = htemp->GetBinContent(bint,bina);
	vadc = hamin + bina*ampa;
	if (content>0){
	  if (local_min > vadc) local_min = vadc;
	  if (local_max < vadc) local_max = vadc;
	}
	npoints += content;
	mean += content * vadc;
	// cout<< content <<" ";
      }
      mean = (npoints>0)?mean/npoints:0;
      local_relerror = (mean>0)?(0.+local_max-local_min)/mean/2:0;
      if (local_relerror > worst_relerror){
	worst_relerror = local_relerror;
	worst_dt = htmin + bint*ampt;
      }
      //cout<<"bint "<< bint <<" npoints: "<<npoints<<" mean: "<< mean <<" loc.min: "<<local_min<<" loc.max: "<<local_max<<" rel.error:"<< local_relerror <<" worst rel.error: "<< global_relerror <<" @ dt: "<< worst_dt <<endl;
      //  cout<<endl;
    }
    
    
    TPaveText *ps = new TPaveText(0.15,0.65,0.45,0.85,"ndc");
    ps->SetTextFont(42);
    ps->SetTextSize(0.03);
    //ps->SetTextColor(kRed);
    sprintf(tmpentry,"Entries = %d",entries); ps->AddText(tmpentry);
    ...
    sprintf(tmpentry,"RMS x = %.3f",RMSx); ps->AddText(tmpentry);
    sprintf(tmpentry,"RMS y = %.3f",RMSy); ps->AddText(tmpentry);
    sprintf(tmpentry,"Worst %s","rel. error"); ps->AddText(tmpentry);
    sprintf(tmpentry,"%.3f %% at dt = %.1f",worst_relerror*100, worst_dt); ps->AddText(tmpentry);
    ps->Draw();  
    
    htemp->SetStats(0);
    
    cout<<endl;
    cout<<"slot "<< adc_slot <<" chan "<< adc_chan <<" worst rel.error: "<< worst_relerror*100 <<" % @ dt: "<< worst_dt <<endl;
  }
  return cWALK1;
}


TCanvas *plot_walk1(int adc_slot, int adc_chan, int tdc_slot=-1, int tdc_chan=-1, int adc_min=0, int adc_max=600, int deltat_min=0, int deltat_max=120){
  TString draw, cut, title;

  MyStyle->SetStatX(0.2);
  MyStyle->SetStatY(0.7);

  TCanvas *cWALK1 = new TCanvas("cWALK1","cWALK1");
