...
// 28-Apr-2015: add support for CAEN sy527 crates: flag CaenSys=true if GUI is connected to CAEN crate. 
//            : add synchronization for monitor suspension (waiting for monitor is in suspended state)
//              in load() methods.
...
public class HVframe  extends JPanel  implements Cmd, Printable, Runnable  {
    ...
    //*** 10 Apr 2015 - for CAEN crates SY527 ****
    public boolean CaenSys = false;
	...
	if(!hvcli.getErrStatus()) {
	    ...
	    wpanel.setPreferredSize(new Dimension(800, 500));
	    ...
	    // for CAEN crates ****
	    if(sysinfo.size()>0) {
		String ssys=sysinfo.toString();
		if(ssys.contains("CAEN"))
		    CaenSys = true;
		else CaenSys = false;
	    }
			   ...
			   &( hvm[k].getSubChnNum().equalsIgnoreCase(hvm[k].getword(moduleInfo[i],1)) ) ) 
{
   ...
   */
	...
	int cnt=0;
	String command = new String("LD "+com.trim());
	//	System.out.println(command);

	// suspend monitor while update modules properties
	hvmon.suspendMonitor(host);
	
	//** 28 Apr 2015 waiting for monitor to be suspended
	if(true)
	do {
	    try{
		Thread.sleep(100);
	 ...
	 } while ( (hvmon.getStatus() !=0 ) && (cnt<100));
	//** 
	...
	if (true) {
		...
		String response =  hvcom.getResponse();
		/* 21 Apr 2015 */
		//	System.out.println( host+": load response=<"+response+">");
	    }		
	}
	}
	// resume monitor
   ...
   public void remove() {
	...
	try {
	    ...
	    if(this.isAlive()) frameThread.interrupt();
	    // wait for thread stop
	    //while(this.isAlive()) {
	    while(!frameThread.currentThread().isInterrupted()){
		try {
		    Thread.sleep(100) ;
		    
		} catch (InterruptedException ie) {
			frameThread.currentThread().interrupt(); 
		} 
	    }
	    this.hvcli.close();
	    if (hvmon.isAlive()) hvmon.stopMonitor();
