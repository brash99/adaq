...
tsCheckAddresses()
{
  ...
  offset = ((unsigned int) &TSp->GTPTriggerTable[0]) - base;
  expected = 0x1080;
  if(offset != expected)
    {
  ...
  offset = ((unsigned int) &TSp->FPTriggerTable[0]) - base;
  expected = 0x1090;
  if(offset != expected)
    {
...
tsSlaveStatus(int pflag)
{
  ...
  if(pflag>0)
    {
      printf(" Registers (offset):\n");
      printf("  TSBase     (0x%08x)\n",TSBase-tsA24Offset);
      printf("  busy           (0x%04x) = 0x%08x\t", (unsigned int)(&TSp->busy) - TSBase, busy);
      printf("  fiber          (0x%04x) = 0x%08x\n", (unsigned int)(&TSp->fiber) - TSBase, fiber);
      printf("  hfbr_tiID[0]   (0x%04x) = 0x%08x\t", (unsigned int)(&TSp->hfbr_tiID[0]) - TSBase, hfbr_tiID[0]);
      printf("  hfbr_tiID[1]   (0x%04x) = 0x%08x\n", (unsigned int)(&TSp->hfbr_tiID[1]) - TSBase, hfbr_tiID[1]);
      printf("  master_tiID    (0x%04x) = 0x%08x\t", (unsigned int)(&TSp->master_tiID) - TSBase, master_tiID);
